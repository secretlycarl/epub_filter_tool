...
Note - It is set up to process 15 books at a time. It runs ok on my beefy PC, but if you run into any performance issues, reduce the ```15```s in both ```def process_folder``` to a lower value.
